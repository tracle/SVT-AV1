    ...
    uint32_t                 fastCandidateTotalCount)
{
...
#if PROOF_OF_CONCEPT_TEST_1
    {
        ...
        context_ptr->bypass_md_stage_2[CAND_CLASS_6] = EB_TRUE;
        context_ptr->bypass_md_stage_2[CAND_CLASS_7] = EB_TRUE;
...
*/
    ...
    // Set md_stage_3 NICs
#if PROOF_OF_CONCEPT_TEST_1
    context_ptr->md_stage_3_count[CAND_CLASS_0] = (picture_control_set_ptr->slice_type == I_SLICE) ? 32 : 4;
#else
    ...
    uint32_t                      *sorted_candidate_index_array)
{
    ...
    uint64_t intra_th = 1;
    if (context_ptr->best_inter_cost < (context_ptr->best_intra_cost * intra_th))
        context_ptr->md_stage_3_total_intra_count = 0;
#endif
#endif
    ...
    uint32_t                      qp)
{
    ...
    for (tx_type = txk_start; tx_type < txk_end; ++tx_type) {
        ...
        av1_estimate_transform(
            ...
            &(((int32_t*)context_ptr->trans_quant_buffers_ptr->tx_buffer_ptr[tx_type]->buffer_y)[context_ptr->txb_1d_offset]),
            NOT_USED_VALUE,
            context_ptr->blk_geom->txsize[context_ptr->tx_depth][context_ptr->txb_itr],
        ...
        tx_type_eob[tx_type] = y_count_non_zero_coeffs;
#endif
        ...
        for (tx_type = txk_start; tx_type < txk_end; ++tx_type) {
...
#endif
            {
                ...
                av1_tu_estimate_coeff_bits(
                    ...
                    COMPONENT_LUMA);
#if NEW_MD_LAMBDA
                cost = RDCOST(full_lambda, y_tu_coeff_bits, tuFullDistortion[0][DIST_CALC_RESIDUAL]);
#else
                uint64_t cost = RDCOST(context_ptr->full_lambda, y_tu_coeff_bits, tuFullDistortion[0][DIST_CALC_RESIDUAL]);
#endif
            }
            ...
            if (context_ptr->blk_geom->sq_size < 128) {
                if (context_ptr->blk_geom->has_uv) {
                    if (candidate_ptr->type == INTRA_MODE) {
                        ...
                        if (context_ptr->best_inter_cost < (context_ptr->best_intra_cost * cfl_th)) {
                            intra_chroma_mode = context_ptr->best_uv_mode[candidate_ptr->intra_luma_mode][MAX_ANGLE_DELTA + candidate_ptr->angle_delta[PLANE_TYPE_Y]];
                            angle_delta = context_ptr->best_uv_angle[candidate_ptr->intra_luma_mode][MAX_ANGLE_DELTA + candidate_ptr->angle_delta[PLANE_TYPE_Y]];
                            is_directional_chroma_mode_flag = (uint8_t)av1_is_directional_mode((PredictionMode)(context_ptr->best_uv_mode[candidate_ptr->intra_luma_mode][MAX_ANGLE_DELTA + candidate_ptr->angle_delta[PLANE_TYPE_Y]]));
                        }
                        else {
                            ...
                            is_directional_chroma_mode_flag = candidate_ptr->intra_chroma_mode != UV_CFL_PRED ?
