...
void block_based_depth_reduction(
    ...
    if (context_ptr->depth_reduction_ctrls.cost_sq_vs_nsq_energy_based_depth_reduction_enabled && context_ptr->best_nsq_default_cost != MAX_MODE_COST) {
#else
...
#if DISALLOW_NSQ_FIX_1
    if (context_ptr->depth_reduction_ctrls.nsq_data_based_depth_reduction_enabled && context_ptr->best_nsq_default_cost != MAX_MODE_COST) {
#else
...
#endif
#if MPPD_PROFILNG
EB_EXTERN EbErrorType pd_pass_0(SequenceControlSet *scs_ptr, PictureControlSet *pcs_ptr,
    ...
    EB_EXTERN EbErrorType pd_pass_1(SequenceControlSet *scs_ptr, PictureControlSet *pcs_ptr,
        const MdcSbData *const mdcResultTbPtr, SuperBlock *sb_ptr,
        uint16_t sb_origin_x, uint16_t sb_origin_y, uint32_t sb_addr,
        ModeDecisionContext *context_ptr) {
        EbErrorType return_error = EB_ErrorNone;
        ...
        EB_EXTERN EbErrorType pd_pass_2(SequenceControlSet *scs_ptr, PictureControlSet *pcs_ptr,
            const MdcSbData *const mdcResultTbPtr, SuperBlock *sb_ptr,
            uint16_t sb_origin_x, uint16_t sb_origin_y, uint32_t sb_addr,
            ModeDecisionContext *context_ptr) {
            EbErrorType return_error = EB_ErrorNone;
