...
EbErrorType av1_inter_prediction(
    ...
    if(picture_control_set_ptr != NULL) {
        ...
        for (uint8_t list_index = REF_LIST_0; list_index <= num_of_list_to_search; ++list_index) {
            ...
            for (ref_pic_index = 0; ref_pic_index < num_of_ref_pic_to_search; ++ref_pic_index) {
                ...
                reference_object = (EbReferenceObject *) picture_control_set_ptr->ref_pic_ptr_array[list_index][ref_pic_index]
                        ->object_ptr;

                EbPictureBufferDesc *ref_pic_ptr = is16bit ? reference_object->reference_picture16bit
                                                           : reference_object->reference_picture;

                av1_setup_scale_factors_for_frame(&(scale_factors[list_index][ref_pic_index]),
                                                  ref_pic_ptr->width,
                                                  ref_pic_ptr->height,
                                                  picture_control_set_ptr->parent_pcs_ptr->enhanced_picture_ptr->width,
                                                  picture_control_set_ptr->parent_pcs_ptr->enhanced_picture_ptr->height);
        ...
        if (sub8x8_inter) {
            ...
            for (int32_t y = 0; y < b8_h; y += b4_h) {
                ...
                for (int32_t x = 0; x < b8_w; x += b4_w) {
                    ...
                    uint8_t list_index = this_mbmi->block_mi.ref_frame[0] == LAST_FRAME ||
                                       this_mbmi->block_mi.ref_frame[0] == LAST2_FRAME ||
                                       this_mbmi->block_mi.ref_frame[0] == LAST3_FRAME ||
                                       this_mbmi->block_mi.ref_frame[0] == GOLDEN_FRAME
                    ...
                    const struct ScaleFactors *const sf = use_intrabc ? &sf_identity : &(scale_factors[list_index][ref_idx]);
                    const int32_t is_scaled = av1_is_scaled(sf);

                    if (is_scaled) {

                        printf("is_scaled sub8x8");
    ...
    if (mv_unit->pred_direction == UNI_PRED_LIST_0 || mv_unit->pred_direction == BI_PRED) {
        ...
        /*ScaleFactor*/
        const struct ScaleFactors *const sf = use_intrabc ? &sf_identity : &ref0_scale_factors;
        uint8_t *src_mod;
        SubpelParams subpel_params;
        const int32_t is_scaled = picture_control_set_ptr != EB_NULL ? av1_is_scaled(sf) : EB_FALSE;
    ...
    if (mv_unit->pred_direction == UNI_PRED_LIST_1 || mv_unit->pred_direction == BI_PRED) {
        ...
        uint32_t ss_y = 0;
        /*ScaleFactor*/
        const struct ScaleFactors *const sf = use_intrabc ? &sf_identity : &ref1_scale_factors;
        uint8_t *src_mod;
        SubpelParams subpel_params;
        const int32_t is_scaled = picture_control_set_ptr != EB_NULL ? av1_is_scaled(sf) : EB_FALSE;
        ...
        if (is_scaled) {
            ...
            assert(!(is_compound && is_masked_compound_type(interinter_comp->type)));
