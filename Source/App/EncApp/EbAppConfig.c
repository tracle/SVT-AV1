...
ConfigEntry config_entry_options[] = {
    // File I/O
    {SINGLE_INPUT, HELP_TOKEN, "Show usage options and exit", option, set_cfg_input_file},
    {SINGLE_INPUT, INPUT_FILE_TOKEN, "Input filename", option, set_cfg_input_file},
    {SINGLE_INPUT, INPUT_FILE_LONG_TOKEN, "Input filename", option, set_cfg_input_file},

    {SINGLE_INPUT, OUTPUT_BITSTREAM_TOKEN, "Output filename", option, set_cfg_stream_file},
    {SINGLE_INPUT, OUTPUT_BITSTREAM_LONG_TOKEN, "Output filename", option, set_cfg_stream_file},

    {SINGLE_INPUT, ERROR_FILE_TOKEN, "Error filename", option, set_cfg_error_file},
    {SINGLE_INPUT, OUTPUT_RECON_TOKEN, "Recon filename", option, set_cfg_recon_file},
    {SINGLE_INPUT, OUTPUT_RECON_LONG_TOKEN, "Recon filename", option, set_cfg_recon_file},

    {SINGLE_INPUT, STAT_FILE_TOKEN, "Stat filename", option, set_cfg_stat_file},
    {SINGLE_INPUT, NULL, NULL, -1, NULL}};

ConfigEntry config_entry_global_options[] = {
    // Picture Dimensions
    {SINGLE_INPUT, WIDTH_TOKEN, "Frame width", global_option, set_cfg_source_width},
    {SINGLE_INPUT, WIDTH_LONG_TOKEN, "Frame width", global_option, set_cfg_source_width},

    {SINGLE_INPUT, HEIGHT_TOKEN, "Frame height", global_option, set_cfg_source_height},
    {SINGLE_INPUT, HEIGHT_LONG_TOKEN, "Frame height", global_option, set_cfg_source_height},
...
ConfigEntry config_entry_rc[] = {
    ...
    {SINGLE_INPUT,
     ...
     rate_control,
     set_cfg_use_qp_file},
    {SINGLE_INPUT, QP_FILE_NEW_TOKEN, "Path to Qp file", rate_control, set_cfg_qp_file},
    {SINGLE_INPUT,
     MAX_QP_TOKEN,
    ...
    {SINGLE_INPUT, VBV_BUFSIZE_TOKEN, "VBV buffer size", rate_control, set_vbv_buf_size},
    // Termination
    {SINGLE_INPUT, NULL, NULL, -1, NULL}};
...
ConfigEntry config_entry_specific[] = {
    ...
    {SINGLE_INPUT, PRESET_TOKEN, "Encoder mode/Preset used[0-8]", specific, set_enc_mode},
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, TILE_ROW_TOKEN, "Number of tile rows to use, log2[0-6]", specific, set_tile_row},
    {SINGLE_INPUT,
     TILE_COL_TOKEN,
     "Number of tile columns to use, log2[0-6]",
     specific,
     set_tile_col},
    {SINGLE_INPUT, QP_TOKEN, "Constant/Constrained Quality level", specific, set_cfg_qp},
    {SINGLE_INPUT, QP_LONG_TOKEN, "Constant/Constrained Quality level", specific, set_cfg_qp},
    ...
    {SINGLE_INPUT, STAT_REPORT_NEW_TOKEN, "Stat Report", specific, set_stat_report},
    {SINGLE_INPUT,
...
ConfigEntry config_entry[] = {
    /***** Options *****/
    {SINGLE_INPUT, HELP_TOKEN, "Show usage options and exit", -1, NULL},
    // File I/O
    {SINGLE_INPUT, INPUT_FILE_LONG_TOKEN, "InputFile", option, set_cfg_input_file},
    {SINGLE_INPUT, INPUT_FILE_TOKEN, "InputFile", no_print_in_help_menu, set_cfg_input_file},

    {SINGLE_INPUT, OUTPUT_BITSTREAM_LONG_TOKEN, "StreamFile", option, set_cfg_stream_file},
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, ERROR_FILE_TOKEN, "ErrorFile", option, set_cfg_error_file},
    {SINGLE_INPUT, OUTPUT_RECON_LONG_TOKEN, "Recon File", option, set_cfg_recon_file},
    {SINGLE_INPUT, OUTPUT_RECON_TOKEN, "ReconFile", option, set_cfg_recon_file},
    {SINGLE_INPUT, STAT_FILE_TOKEN, "StatFile", option, set_cfg_stat_file},

    /***** Global options *****/
    // Picture Dimensions
    {SINGLE_INPUT, WIDTH_LONG_TOKEN, "SourceWidth", global_option, set_cfg_source_width},
    {SINGLE_INPUT, WIDTH_TOKEN, "SourceWidth", no_print_in_help_menu, set_cfg_source_width},

    {SINGLE_INPUT, HEIGHT_LONG_TOKEN, "SourceHeight", global_option, set_cfg_source_height},
    {SINGLE_INPUT, HEIGHT_TOKEN, "SourceHeight", no_print_in_help_menu, set_cfg_source_height},
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_cfg_frames_to_be_encoded},
    {SINGLE_INPUT, BUFFERED_INPUT_TOKEN, "BufferedInput", global_option, set_buffered_input},
    {SINGLE_INPUT,
     ENCODER_COLOR_FORMAT,
     "EncoderColorFormat",
     global_option,
     set_encoder_color_format},
    {SINGLE_INPUT, PROFILE_TOKEN, "Profile", global_option, set_profile},
    {SINGLE_INPUT, FRAME_RATE_TOKEN, "FrameRate", global_option, set_frame_rate},
    {SINGLE_INPUT,
     FRAME_RATE_NUMERATOR_TOKEN,
     "FrameRateNumerator",
     global_option,
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_encoder_bit_depth},
    {SINGLE_INPUT, INPUT_DEPTH_TOKEN, "Encoder Bit Depth", global_option, set_encoder_bit_depth},
    {SINGLE_INPUT,
     HIERARCHICAL_LEVELS_TOKEN,
     "HierarchicalLevels",
     global_option,
     set_hierarchical_levels},
    {SINGLE_INPUT, PRED_STRUCT_TOKEN, "PredStructure", global_option, set_cfg_pred_structure},
    {SINGLE_INPUT,
     HDR_INPUT_TOKEN,
    ...
    {SINGLE_INPUT,
     ...
     set_high_dynamic_range_input},
    // Asm Type
    {SINGLE_INPUT, ASM_TYPE_TOKEN, "Asm", global_option, set_asm_type},
    // Thread Management
    {SINGLE_INPUT, THREAD_MGMNT, "LogicalProcessors", global_option, set_logical_processors},
    {SINGLE_INPUT,
     UNPIN_LP1_TOKEN,
     "UnpinSingleCoreExecution",
     global_option,
     set_unpin_single_core_execution},
    {SINGLE_INPUT, TARGET_SOCKET, "TargetSocket", global_option, set_target_socket},
    ...
    {SINGLE_INPUT,
     ...
     global_option,
     set_rate_control_mode},
    {SINGLE_INPUT, TARGET_BIT_RATE_TOKEN, "TargetBitRate", rate_control, set_target_bit_rate},
    {SINGLE_INPUT, USE_QP_FILE_TOKEN, "UseQpFile", rate_control, set_cfg_use_qp_file},
    {SINGLE_INPUT, QP_FILE_TOKEN, "QpFile", no_print_in_help_menu, set_cfg_qp_file},
    {SINGLE_INPUT, QP_FILE_NEW_TOKEN, "Path to Qp file", rate_control, set_cfg_qp_file},
    {SINGLE_INPUT, MAX_QP_TOKEN, "MaxQpAllowed", rate_control, set_max_qp_allowed},
    {SINGLE_INPUT, MIN_QP_TOKEN, "MinQpAllowed", rate_control, set_min_qp_allowed},
    {SINGLE_INPUT,
     ADAPTIVE_QP_ENABLE_TOKEN,
    ...
    {SINGLE_INPUT,
     ...
     rate_control,
     set_adaptive_quantization},
    {SINGLE_INPUT, VBV_BUFSIZE_TOKEN, "VBVBufSize", rate_control, set_vbv_buf_size},

    /***** Two pass *****/
    {SINGLE_INPUT, OUTPUT_STAT_FILE_TOKEN, "output_stat_file", two_pass, set_output_stat_file},
    {SINGLE_INPUT, INPUT_STAT_FILE_TOKEN, "input_stat_file", two_pass, set_input_stat_file},
    {SINGLE_INPUT, ENCMODE2P_TOKEN, "EncoderMode2p", two_pass, set_snd_pass_enc_mode},

    /***** Intra refresh *****/
    {SINGLE_INPUT, INTRA_PERIOD_TOKEN, "IntraPeriod", no_print_in_help_menu, set_cfg_intra_period},
    {SINGLE_INPUT, KEYINT_TOKEN, "Intra Period", intra_refresh, set_cfg_intra_period},
    {SINGLE_INPUT,
     INTRA_REFRESH_TYPE_TOKEN,
     "IntraRefreshType",
     intra_refresh,
    ...
    /***** Specific *****/
    {SINGLE_INPUT, PRESET_TOKEN, "Encoder mode/Preset used[0-8]", specific, set_enc_mode},
    {SINGLE_INPUT, ENCMODE_TOKEN, "EncoderMode", no_print_in_help_menu, set_enc_mode},
    {SINGLE_INPUT, INPUT_PREDSTRUCT_FILE_TOKEN, "pred_struct_file", specific, set_pred_struct_file},

    {SINGLE_INPUT, ENCODER_BIT_DEPTH, "EncoderBitDepth", specific, set_encoder_bit_depth},
    {SINGLE_INPUT,
     ENCODER_16BIT_PIPELINE,
     "Encoder16BitPipeline",
     specific,
     set_encoder_16bit_pipeline},
    {SINGLE_INPUT, ENCODER_COLOR_FORMAT, "EncoderColorFormat", specific, set_encoder_color_format},
    {SINGLE_INPUT,
     INPUT_COMPRESSED_TEN_BIT_FORMAT,
     "CompressedTenBitFormat",
     specific,
     set_compressed_ten_bit_format},
    {SINGLE_INPUT, TILE_ROW_TOKEN, "TileRow", specific, set_tile_row},
    {SINGLE_INPUT, TILE_COL_TOKEN, "TileCol", specific, set_tile_col},
    // Rate Control
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, STAT_REPORT_TOKEN, "StatReport", no_print_in_help_menu, set_stat_report},
    {SINGLE_INPUT, STAT_REPORT_NEW_TOKEN, "Stat Report", specific, set_stat_report},
    {SINGLE_INPUT, QP_LONG_TOKEN, "QP", specific, set_cfg_qp},
    {SINGLE_INPUT, QP_TOKEN, "QP", no_print_in_help_menu, set_cfg_qp},
    {SINGLE_INPUT,
     LOOK_AHEAD_DIST_TOKEN,
     "LookAheadDistance",
     no_print_in_help_menu,
     set_look_ahead_distance},
    {SINGLE_INPUT, LOOKAHEAD_NEW_TOKEN, "Look Ahead Distance", specific, set_look_ahead_distance},
    ...
    // CDEF
    {SINGLE_INPUT, CDEF_MODE_TOKEN, "CDEFMode", specific, set_cdef_mode},
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_enable_restoration_filter_flag},
    {SINGLE_INPUT, SG_FILTER_MODE_TOKEN, "SelfGuidedFilterMode", specific, set_sg_filter_mode},
    {SINGLE_INPUT, WN_FILTER_MODE_TOKEN, "WienerFilterMode", specific, set_wn_filter_mode},

    {SINGLE_INPUT, MFMV_ENABLE_TOKEN, "Mfmv", no_print_in_help_menu, set_enable_mfmv_flag},
    {SINGLE_INPUT,
     MFMV_ENABLE_NEW_TOKEN,
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_spatial_sse_fl_flag},
    {SINGLE_INPUT, SPATIAL_SSE_FL_NEW_TOKEN, "Spatial SSE fl", specific, set_spatial_sse_fl_flag},
    {SINGLE_INPUT, SUBPEL_TOKEN, "Subpel", specific, set_enable_sub_pel_flag},
    {SINGLE_INPUT,
     OVR_BNDRY_BLK_TOKEN,
     "OverBoundryBlock",
     no_print_in_help_menu,
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_prune_ref_rec_part_flag},
    {SINGLE_INPUT, NSQ_TABLE_TOKEN, "NsqTable", no_print_in_help_menu, set_nsq_table_flag},
    {SINGLE_INPUT, NSQ_TABLE_NEW_TOKEN, "Nsq Table", specific, set_nsq_table_flag},
    {SINGLE_INPUT,
    ...
    // CHROMA
    {SINGLE_INPUT, CHROMA_MODE_TOKEN, "ChromaMode", specific, set_chroma_mode},
    {SINGLE_INPUT, DISABLE_CFL_TOKEN, "DisableCFL", no_print_in_help_menu, set_disable_cfl_flag},
    {SINGLE_INPUT, DISABLE_CFL_NEW_TOKEN, "Disable CFL", specific, set_disable_cfl_flag},
    ...
    // CLASS 12
    {SINGLE_INPUT, CLASS_12_TOKEN, "CombineClass12", specific, set_class_12_flag},
    // EDGE SKIP ANGLE INTRA
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT,
     ...
     set_interintra_compound_flag},
    // PAETH
    {SINGLE_INPUT, PAETH_TOKEN, "Paeth", no_print_in_help_menu, set_enable_paeth_flag},
    {SINGLE_INPUT, PAETH_NEW_TOKEN, "Paeth New Token", specific, set_enable_paeth_flag},
    // SMOOTH
    {SINGLE_INPUT, SMOOTH_TOKEN, "Smooth", specific, set_enable_smooth_flag},
    {SINGLE_INPUT, SMOOTH_NEW_TOKEN, "Smooth New Token", specific, set_enable_smooth_flag},
    // OBMC
    {SINGLE_INPUT, OBMC_TOKEN, "Obmc", specific, set_enable_obmc_flag},
    // RDOQ
    {SINGLE_INPUT, RDOQ_TOKEN, "RDOQ", no_print_in_help_menu, set_enable_rdoq_flag},
    {SINGLE_INPUT, RDOQ_NEW_TOKEN, "RDOQ double dash token", specific, set_enable_rdoq_flag},
    // Filter Intra
    {SINGLE_INPUT,
     FILTER_INTRA_TOKEN,
     "FilterIntra",
     no_print_in_help_menu,
     set_enable_filter_intra_flag},
    {SINGLE_INPUT, FILTER_INTRA_NEW_TOKEN, "Filter Intra", specific, set_enable_filter_intra_flag},
    ...
    // Picture based rate estimation
    {SINGLE_INPUT, PIC_BASED_RATE_EST_TOKEN, "PicBasedRateEst", specific, set_pic_based_rate_est},

    // PREDICTIVE ME
    {SINGLE_INPUT, PRED_ME_TOKEN, "PredMe", specific, set_predictive_me_flag},
    // BIPRED 3x3 INJECTION
    {SINGLE_INPUT, BIPRED_3x3_TOKEN, "Bipred3x3", specific, set_bipred3x3inject_flag},
    // COMPOUND MODE
    {SINGLE_INPUT, COMPOUND_LEVEL_TOKEN, "CompoundLevel", specific, set_compound_level_flag},
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_cfg_use_default_me_hme},
    {SINGLE_INPUT, HME_ENABLE_TOKEN, "HME", specific, set_enable_hme_flag},
    {SINGLE_INPUT, HME_L0_ENABLE_TOKEN, "HMELevel0", specific, set_enable_hme_level_0_flag},
    {SINGLE_INPUT, HME_L1_ENABLE_TOKEN, "HMELevel1", specific, set_enable_hme_level_1_flag},
    {SINGLE_INPUT, HME_L2_ENABLE_TOKEN, "HMELevel2", specific, set_enable_hme_level_2_flag},
    {SINGLE_INPUT, EXT_BLOCK, "ExtBlockFlag", specific, set_enable_ext_block_flag},
    // ME Parameters
    {SINGLE_INPUT, SEARCH_AREA_WIDTH_TOKEN, "SearchAreaWidth", specific, set_cfg_search_area_width},
    {SINGLE_INPUT,
     SEARCH_AREA_HEIGHT_TOKEN,
     "SearchAreaHeight",
     specific,
    ...
    {SINGLE_INPUT,
     ...
     set_cfg_hme_level_0_total_search_area_height},
    // MD Parameters
    {SINGLE_INPUT, SCREEN_CONTENT_TOKEN, "ScreenContentMode", specific, set_screen_content_mode},
    {SINGLE_INPUT,
     HBD_MD_ENABLE_TOKEN,
     "HighBitDepthModeDecision",
     specific,
     set_enable_hbd_mode_decision},
    {SINGLE_INPUT, PALETTE_TOKEN, "PaletteMode", specific, set_enable_palette},
    {SINGLE_INPUT, OLPD_REFINEMENT_TOKEN, "OlpdRefinement", specific, set_enable_olpd_refinement},
    ...
    // Latency
    {SINGLE_INPUT, INJECTOR_TOKEN, "Injector", specific, set_injector},
    {SINGLE_INPUT,
     INJECTOR_FRAMERATE_TOKEN,
     "InjectorFrameRate",
     specific,
     set_injector_frame_rate},
    {SINGLE_INPUT, SPEED_CONTROL_TOKEN, "SpeedControlFlag", specific, speed_control_flag},
    // Annex A parameters
    {SINGLE_INPUT, PROFILE_TOKEN, "Profile", specific, set_profile},
    {SINGLE_INPUT, TIER_TOKEN, "Tier", specific, set_tier},
    {SINGLE_INPUT, LEVEL_TOKEN, "Level", specific, set_level},
    {SINGLE_INPUT, LATENCY_MODE, "LatencyMode", specific, set_latency_mode},
    {SINGLE_INPUT, FILM_GRAIN_TOKEN, "FilmGrain", specific, set_cfg_film_grain},
    ...
    {ARRAY_INPUT,
     ...
     set_hme_level_2_search_area_in_height_array},
    // --- start: ALTREF_FILTERING_SUPPORT
    {SINGLE_INPUT, ENABLE_ALTREFS, "EnableAltRefs", specific, set_enable_altrefs},
    {SINGLE_INPUT, ALTREF_STRENGTH, "AltRefStrength", specific, set_altref_strength},
    {SINGLE_INPUT, ALTREF_NFRAMES, "AltRefNframes", specific, set_altref_n_frames},
    {SINGLE_INPUT, ENABLE_OVERLAYS, "EnableOverlays", specific, set_enable_overlays},
    // --- end: ALTREF_FILTERING_SUPPORT
    // Super-resolution support
    {SINGLE_INPUT, SUPERRES_MODE_INPUT, "SuperresMode", specific, set_superres_mode},
    {SINGLE_INPUT, SUPERRES_DENOM, "SuperresDenom", specific, set_superres_denom},
    {SINGLE_INPUT, SUPERRES_KF_DENOM, "SuperresKfDenom", specific, set_superres_kf_denom},
    {SINGLE_INPUT, SUPERRES_QTHRES, "SuperresQthres", specific, set_superres_qthres},

    {SINGLE_INPUT, SQ_WEIGHT_TOKEN, "SquareWeight", specific, set_square_weight},
    {SINGLE_INPUT,
     MDS_1_PRUNE_C_TH,
...
uint32_t check_long(ConfigEntry config_entry, ConfigEntry config_entry_next) {
    ...
    //fprintf(stderr, "config_entry: %s\n", config_entry.name);
    //fprintf(stderr, "config_entry_next: %s\n", config_entry_next.name);
...
uint32_t get_help(int32_t argc, char *const argv[]) {
    ...
    if (find_token(argc, argv, HELP_TOKEN, config_string) == 0 ||
        ...
        while (config_entry[++i].name != NULL) {
            ...
            //fprintf(stderr, "config_entry: %d\n", prev_enum_option);
            //fprintf(stderr, "config_entry_next: %d\n", curr_enum_option);
            // Print the options
            ...
            if (check == 1) {
                //if (config_entry[i].opt == no_print_in_help_menu) continue;
                ...
                if (*(config_entry[i].token + 1) == '-')
                    ...
                    token_format = "\t%-5s\t-%-25s\t%-25s\n";
                fprintf(stderr, token_format, "", config_entry[i].token, config_entry[i].name);
            }
        }
        return 1;
    } else {
