...
ConfigEntry config_entry_options[] = {
    // File I/O
    {SINGLE_INPUT, HELP_TOKEN, "Show usage options and exit", NULL, option, set_cfg_input_file},
    {SINGLE_INPUT, INPUT_FILE_TOKEN, "Input filename", NULL, option, set_cfg_input_file},
    {SINGLE_INPUT, INPUT_FILE_LONG_TOKEN, "Input filename", NULL, option, set_cfg_input_file},

    {SINGLE_INPUT, OUTPUT_BITSTREAM_TOKEN, "Output filename", NULL, option, set_cfg_stream_file},
    {SINGLE_INPUT, OUTPUT_BITSTREAM_LONG_TOKEN, "Output filename", NULL, option, set_cfg_stream_file},

    {SINGLE_INPUT, ERROR_FILE_TOKEN, "Error filename", NULL, option, set_cfg_error_file},
    {SINGLE_INPUT, OUTPUT_RECON_TOKEN, "Recon filename", NULL, option, set_cfg_recon_file},
    {SINGLE_INPUT, OUTPUT_RECON_LONG_TOKEN, "Recon filename", NULL, option, set_cfg_recon_file},

    {SINGLE_INPUT, STAT_FILE_TOKEN, "Stat filename", NULL, option, set_cfg_stat_file},
    {SINGLE_INPUT, NULL, NULL, NULL, -1, NULL}};

ConfigEntry config_entry_global_options[] = {
    // Picture Dimensions
    {SINGLE_INPUT, WIDTH_TOKEN, "Frame width", NULL, global_option, set_cfg_source_width},
    {SINGLE_INPUT, WIDTH_LONG_TOKEN, "Frame width", NULL, global_option, set_cfg_source_width},

    {SINGLE_INPUT, HEIGHT_TOKEN, "Frame height", NULL, global_option, set_cfg_source_height},
    {SINGLE_INPUT, HEIGHT_LONG_TOKEN, "Frame height", NULL, global_option, set_cfg_source_height},
...
ConfigEntry config_entry_rc[] = {
    ...
    {SINGLE_INPUT,
     ...
     rate_control,
     set_cfg_use_qp_file},
    {SINGLE_INPUT, QP_FILE_NEW_TOKEN, "Path to Qp file", NULL, rate_control, set_cfg_qp_file},
    {SINGLE_INPUT,
     MAX_QP_TOKEN,
    ...
    {SINGLE_INPUT, VBV_BUFSIZE_TOKEN, "VBV buffer size", NULL, rate_control, set_vbv_buf_size},
    // Termination
    {SINGLE_INPUT, NULL, NULL, NULL, -1, NULL}};
...
ConfigEntry config_entry_specific[] = {
    ...
    {SINGLE_INPUT, PRESET_TOKEN, "Encoder mode/Preset used[0-8]", NULL, specific, set_enc_mode},
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, TILE_ROW_TOKEN, "Number of tile rows to use, log2[0-6]", NULL, specific, set_tile_row},
    {SINGLE_INPUT,
     TILE_COL_TOKEN,
     ...
     specific,
     set_tile_col},
    {SINGLE_INPUT, QP_TOKEN, "Constant/Constrained Quality level", NULL, specific, set_cfg_qp},
    {SINGLE_INPUT, QP_LONG_TOKEN, "Constant/Constrained Quality level", NULL, specific, set_cfg_qp},
    ...
    {SINGLE_INPUT, STAT_REPORT_NEW_TOKEN, "Stat Report", NULL, specific, set_stat_report},
    {SINGLE_INPUT,
...
/**********************************
List with all the comfig entry
*********************************/
ConfigEntry config_entry[] = {
    ...
    {SINGLE_INPUT, OUTPUT_RECON_TOKEN, "ReconFile", NULL, option, set_cfg_recon_file},
    {SINGLE_INPUT, STAT_FILE_TOKEN, "StatFile", "Stat filename", option, set_cfg_stat_file},
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_cfg_frames_to_be_encoded},
    {SINGLE_INPUT, BUFFERED_INPUT_TOKEN, "BufferedInput", "Buffer n input frames", global_option, set_buffered_input},
    {SINGLE_INPUT,
     ENCODER_COLOR_FORMAT,
     "EncoderColorFormat",
     "Set encoder color format(EB_YUV400, EB_YUV420, EB_YUV422, EB_YUV444)",
    ...
    {SINGLE_INPUT,
     ...
     global_option,
     set_profile},
    {SINGLE_INPUT, FRAME_RATE_TOKEN, "FrameRate", "Stream frame rate (rate/scale)", global_option, set_frame_rate},
    {SINGLE_INPUT,
     FRAME_RATE_NUMERATOR_TOKEN,
     "FrameRateNumerator",
     "Stream frame rate numerator",
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_encoder_bit_depth},
    {SINGLE_INPUT, INPUT_DEPTH_TOKEN, "Encoder Bit Depth", "Bit depth for codec(8 or 10)", global_option, set_encoder_bit_depth},
    {SINGLE_INPUT,
     HIERARCHICAL_LEVELS_TOKEN,
     "HierarchicalLevels",
     "Set hierarchical levels(3 or 4[default])",
    ...
    {SINGLE_INPUT,
     ...
     set_asm_type},
    // Thread Management
    {SINGLE_INPUT, THREAD_MGMNT, "LogicalProcessors", "number of logical processors to be used", global_option, set_logical_processors},
    {SINGLE_INPUT,
     UNPIN_LP1_TOKEN,
     ...
     global_option,
     set_unpin_single_core_execution},
    {SINGLE_INPUT, TARGET_SOCKET, "TargetSocket", "Specify  which socket the encoder runs on", global_option, set_target_socket},
    ...
    {SINGLE_INPUT,
     ...
     global_option,
     set_rate_control_mode},
    {SINGLE_INPUT, TARGET_BIT_RATE_TOKEN, "TargetBitRate", "Target Bitrate (kbps)", rate_control, set_target_bit_rate},
    {SINGLE_INPUT,
     USE_QP_FILE_TOKEN,
     "UseQpFile",
     "Overwrite QP assignment using qp values in QP file",
     rate_control,
     set_cfg_use_qp_file},
    {SINGLE_INPUT, QP_FILE_TOKEN, "QpFile", NULL, no_print_in_help_menu, set_cfg_qp_file},
    {SINGLE_INPUT, QP_FILE_NEW_TOKEN, "Path to Qp file", "Path to Qp file", rate_control, set_cfg_qp_file},
    {SINGLE_INPUT, MAX_QP_TOKEN, "MaxQpAllowed", "Maximum (worst) quantizer[0-63]", rate_control, set_max_qp_allowed},
    {SINGLE_INPUT, MIN_QP_TOKEN, "MinQpAllowed", "Minimum (best) quantizer[0-63]", rate_control, set_min_qp_allowed},
    {SINGLE_INPUT,
     ADAPTIVE_QP_ENABLE_TOKEN,
    ...
    {SINGLE_INPUT,
     ...
     rate_control,
     set_adaptive_quantization},
    {SINGLE_INPUT, VBV_BUFSIZE_TOKEN, "VBVBufSize", "VBV buffer size", rate_control, set_vbv_buf_size},

    /***** Two pass *****/
    {SINGLE_INPUT, OUTPUT_STAT_FILE_TOKEN, "output_stat_file", "First pass stat file output", two_pass, set_output_stat_file},
    {SINGLE_INPUT, INPUT_STAT_FILE_TOKEN, "input_stat_file", "Input the first pass output to the second pass", two_pass, set_input_stat_file},
    {SINGLE_INPUT, ENCMODE2P_TOKEN, "EncoderMode2p", "Use Hme/Me settings of the second pass'encoder mode in the first pass", two_pass, set_snd_pass_enc_mode},

    /***** Intra refresh *****/
    {SINGLE_INPUT, INTRA_PERIOD_TOKEN, "IntraPeriod", NULL, no_print_in_help_menu, set_cfg_intra_period},
    {SINGLE_INPUT, KEYINT_TOKEN, "Intra Period", "Intra period interval(frames) (-2: No intra update, -1: default intra period or [0-255])", intra_refresh, set_cfg_intra_period},
    {SINGLE_INPUT,
     INTRA_REFRESH_TYPE_TOKEN,
     "IntraRefreshType",
     "Intra refresh type (1: CRA (Open GOP)2: IDR (Closed GOP))",
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_enc_mode},
    {SINGLE_INPUT, ENCMODE_TOKEN, "EncoderMode", NULL, no_print_in_help_menu, set_enc_mode},
    {SINGLE_INPUT, INPUT_PREDSTRUCT_FILE_TOKEN, "pred_struct_file", NULL, specific, set_pred_struct_file},
    ...
    {SINGLE_INPUT, ENCODER_COLOR_FORMAT, "EncoderColorFormat", NULL, specific, set_encoder_color_format},
    {SINGLE_INPUT,
     INPUT_COMPRESSED_TEN_BIT_FORMAT,
     "CompressedTenBitFormat",
     "Offline packing of the 2bits: requires two bits packed input (0: OFF[default], 1: "
     "ON)",
     specific,
     set_compressed_ten_bit_format},
    {SINGLE_INPUT, TILE_ROW_TOKEN, "TileRow", "Number of tile rows to use, log2[0-6]", specific, set_tile_row},
    {SINGLE_INPUT, TILE_COL_TOKEN, "TileCol", "Number of tile columns to use, log2[0-6]", specific, set_tile_col},
    // Rate Control
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, STAT_REPORT_TOKEN, "StatReport", NULL, no_print_in_help_menu, set_stat_report},
    {SINGLE_INPUT, STAT_REPORT_NEW_TOKEN, "Stat Report", "Stat Report", specific, set_stat_report},
    {SINGLE_INPUT, QP_LONG_TOKEN, "QP", "Constant/Constrained Quality level", specific, set_cfg_qp},
    {SINGLE_INPUT, QP_TOKEN, "QP", NULL, no_print_in_help_menu, set_cfg_qp},
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT, MFMV_ENABLE_TOKEN, "Mfmv", NULL, no_print_in_help_menu, set_enable_mfmv_flag},
    {SINGLE_INPUT,
     MFMV_ENABLE_NEW_TOKEN,
     "Mfmv token with double dash",
     "Enable motion field motion vector( 0: OFF, 1: ON, -1: DEFAULT)",
     specific,
     set_enable_mfmv_flag},
    {SINGLE_INPUT,
    ...
    {SINGLE_INPUT,
     ...
     no_print_in_help_menu,
     set_spatial_sse_fl_flag},
    {SINGLE_INPUT, SPATIAL_SSE_FL_NEW_TOKEN, "Spatial SSE fl", "Enable spatial sse full loop(0: OFF, 1: ON, -1: DEFAULT)", specific, set_spatial_sse_fl_flag},
    {SINGLE_INPUT, SUBPEL_TOKEN, "Subpel", "Enable subpel(0: OFF, 1: ON, -1: DEFAULT)", specific, set_enable_sub_pel_flag},
    {SINGLE_INPUT,
     OVR_BNDRY_BLK_TOKEN,
     "OverBoundryBlock",
     NULL,
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_prune_ref_rec_part_flag},
    {SINGLE_INPUT, NSQ_TABLE_TOKEN, "NsqTable", NULL, no_print_in_help_menu, set_nsq_table_flag},
    {SINGLE_INPUT, NSQ_TABLE_NEW_TOKEN, "Nsq Table", "Enable nsq table (0: OFF, 1: ON, -1: DEFAULT)", specific, set_nsq_table_flag},
    {SINGLE_INPUT,
    ...
    // CHROMA
    {SINGLE_INPUT, CHROMA_MODE_TOKEN, "ChromaMode", "Select chroma mode([0-3], -1: DEFAULT)", specific, set_chroma_mode},
    {SINGLE_INPUT, DISABLE_CFL_TOKEN, "DisableCFL", NULL, no_print_in_help_menu, set_disable_cfl_flag},
    {SINGLE_INPUT, DISABLE_CFL_NEW_TOKEN, "Disable CFL", "Set chroma from luma (CFL) flag (0: OFF, 1: ON, -1: DEFAULT)", specific, set_disable_cfl_flag},
    ...
    // CLASS 12
    {SINGLE_INPUT, CLASS_12_TOKEN, "CombineClass12", NULL, specific, set_class_12_flag},
    {SINGLE_INPUT,
     CLASS_12_NEW_TOKEN,
     "Combine Class 12",
     "Enable combine MD Class1&2 (0: OFF, 1: ON, -1: DEFAULT)",
    ...
    {SINGLE_INPUT,
     ...
     set_interintra_compound_flag},
    // PAETH
    {SINGLE_INPUT, PAETH_TOKEN, "Paeth", NULL, no_print_in_help_menu, set_enable_paeth_flag},
    {SINGLE_INPUT, PAETH_NEW_TOKEN, "Paeth New Token", "Enable paeth (0: OFF, 1: ON, -1: DEFAULT)", specific, set_enable_paeth_flag},
    // SMOOTH
    {SINGLE_INPUT, SMOOTH_TOKEN, "Smooth", NULL, specific, set_enable_smooth_flag},
    {SINGLE_INPUT,
     SMOOTH_NEW_TOKEN,
     "Smooth New Token",
     "Enable smooth (0: OFF, 1: ON, -1: DEFAULT)",
     ...
     set_enable_smooth_flag},
    // OBMC
    {SINGLE_INPUT, OBMC_TOKEN, "Obmc", "Enable OBMC(0: OFF, 1: ON[default]) ", specific, set_enable_obmc_flag},
    // RDOQ
    {SINGLE_INPUT, RDOQ_TOKEN, "RDOQ", NULL, no_print_in_help_menu, set_enable_rdoq_flag},
    {SINGLE_INPUT,
     RDOQ_NEW_TOKEN,
     "RDOQ double dash token",
     "Enable RDOQ (0: OFF, 1: ON, -1: DEFAULT)",
     specific,
     set_enable_rdoq_flag},
    // Filter Intra
    ...
    // PREDICTIVE ME
    {SINGLE_INPUT, PRED_ME_TOKEN, "PredMe", "Set predictive motion estimation level(-1: default, [0-5])", specific, set_predictive_me_flag},
    // BIPRED 3x3 INJECTION
    {SINGLE_INPUT, BIPRED_3x3_TOKEN, "Bipred3x3", "Set bipred3x3 injection (0: OFF, 1: ON FULL, 2: Reduced set, -1: DEFAULT)", specific, set_bipred3x3inject_flag},
    // COMPOUND MODE
    {SINGLE_INPUT, COMPOUND_LEVEL_TOKEN, "CompoundLevel", "Enable compound mode(0: OFF, 1:ON[AVG/DIST/DIFF], 2: ON[AVG/DIST/DIFF/WEDGE], -1: default)", specific, set_compound_level_flag},
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_cfg_use_default_me_hme},
    {SINGLE_INPUT, HME_ENABLE_TOKEN, "HME", "Enable hierarchical motion estimation(0: OFF, 1: ON)", specific, set_enable_hme_flag},
    {SINGLE_INPUT,
     HME_L0_ENABLE_TOKEN,
     "HMELevel0",
     "Enable hierarchical motion estimation Level 0 (0: OFF, 1: ON)",
     specific,
     set_enable_hme_level_0_flag},
    {SINGLE_INPUT, HME_L1_ENABLE_TOKEN, "HMELevel1", "Enable hierarchical motion estimation Level 1 (0: OFF, 1: ON)", specific, set_enable_hme_level_1_flag},
    {SINGLE_INPUT, HME_L2_ENABLE_TOKEN, "HMELevel2", "Enable hierarchical motion estimation Level 2 (0: OFF, 1: ON)", specific, set_enable_hme_level_2_flag},
    {SINGLE_INPUT,
     EXT_BLOCK,
     "ExtBlockFlag",
     "Enable the rectangular and asymetric block (0: OFF, 1: ON)",
    ...
    {SINGLE_INPUT,
     ...
     set_cfg_hme_level_0_total_search_area_height},
    // MD Parameters
    {SINGLE_INPUT, SCREEN_CONTENT_TOKEN, "ScreenContentMode", "Set screen content detection level([0-2], 2: DEFAULT)", specific, set_screen_content_mode},
    {SINGLE_INPUT,
     HBD_MD_ENABLE_TOKEN,
     ...
     specific,
     set_enable_hbd_mode_decision},
    {SINGLE_INPUT, PALETTE_TOKEN, "PaletteMode", "Set palette prediction mode(-1: default or [0-6])", specific, set_enable_palette},
    {SINGLE_INPUT, OLPD_REFINEMENT_TOKEN, "OlpdRefinement", NULL, specific, set_enable_olpd_refinement},

    // Optional Features
    {SINGLE_INPUT,
     UNRESTRICTED_MOTION_VECTOR,
     "UnrestrictedMotionVector",
     "Allow motion vectors to reach outside of the picture boundary(O: OFF, 1: "
     "ON[default])",
     specific,
     set_unrestricted_motion_vector},

    //    { SINGLE_INPUT, BITRATE_REDUCTION_TOKEN, "bit_rate_reduction", specific, setBitRateReduction },
    // Latency
    {SINGLE_INPUT,
     INJECTOR_TOKEN,
     "Injector",
    ...
    {SINGLE_INPUT,
     ...
     specific,
     set_injector_frame_rate},
    {SINGLE_INPUT, SPEED_CONTROL_TOKEN, "SpeedControlFlag", "Enable speed control(0: OFF[default], 1: ON)", specific, speed_control_flag},
    // Annex A parameters
    {SINGLE_INPUT, PROFILE_TOKEN, "Profile", NULL, specific, set_profile},
    {SINGLE_INPUT, TIER_TOKEN, "Tier", NULL, specific, set_tier},
    {SINGLE_INPUT, LEVEL_TOKEN, "Level", NULL, specific, set_level},
    {SINGLE_INPUT, LATENCY_MODE, "LatencyMode", NULL, specific, set_latency_mode},
    {SINGLE_INPUT, FILM_GRAIN_TOKEN, "FilmGrain", "Enable film grain(0: OFF[default], 1-50: ON, film-grain denoising strength)", specific, set_cfg_film_grain},
    ...
    {ARRAY_INPUT,
     ...
     set_hme_level_2_search_area_in_height_array},
    // --- start: ALTREF_FILTERING_SUPPORT
    {SINGLE_INPUT, ENABLE_ALTREFS, "EnableAltRefs", "Enable automatic alt reference frames(0: OFF, 1: ON[default])", specific, set_enable_altrefs},
    {SINGLE_INPUT, ALTREF_STRENGTH, "AltRefStrength", "AltRef filter strength([0-6], default: 5)", specific, set_altref_strength},
    {SINGLE_INPUT, ALTREF_NFRAMES, "AltRefNframes", "AltRef max frames([0-10], default: 7)", specific, set_altref_n_frames},
    {SINGLE_INPUT, ENABLE_OVERLAYS, "EnableOverlays", "Enable the insertion of an extra picture called overlayer picture which will be used as an extra reference frame for the base-layer picture(0: OFF[default], 1: ON)", specific, set_enable_overlays},
    // --- end: ALTREF_FILTERING_SUPPORT
    // Super-resolution support
    {SINGLE_INPUT, SUPERRES_MODE_INPUT, "SuperresMode", NULL, specific, set_superres_mode},
    {SINGLE_INPUT, SUPERRES_DENOM, "SuperresDenom", NULL, specific, set_superres_denom},
    {SINGLE_INPUT, SUPERRES_KF_DENOM, "SuperresKfDenom", NULL, specific, set_superres_kf_denom},
    {SINGLE_INPUT, SUPERRES_QTHRES, "SuperresQthres", NULL, specific, set_superres_qthres},
...
uint32_t get_help(int32_t argc, char *const argv[]) {
    ...
    if (find_token(argc, argv, HELP_TOKEN, config_string) == 0 ||
        ...
        fprintf(stderr,
                ...
                "Usage: SvtAv1EncApp.exe <options> -b dst_filename -i src_filename");
        while (config_entry[++i].name != NULL) {
            curr_enum_option = config_entry[i].opt;
            // Print the options
            if (prev_enum_option != curr_enum_option && curr_enum_option != no_print_in_help_menu) {
                switch (curr_enum_option) {
                case option: fprintf(stderr, "\n%-25s\n", "Options:"); break;
                case global_option: fprintf(stderr, "\n%-25s\n", "Encoder Global Options:"); break;
                case rate_control: fprintf(stderr, "\n%-25s\n", "Rate Control Options:"); break;
                case two_pass: fprintf(stderr, "\n%-25s\n", "Twopass Options:"); break;
                case intra_refresh:
                    fprintf(stderr, "\n%-25s\n", "Keyframe Placement Options:");
                    break;
                case specific: fprintf(stderr, "\n%-25s\n", "AV1 Specific Options:"); break;
            ...
            //Print the tokens
            int check = check_long(config_entry[i], config_entry[i + 1]) == 1;
            //fprintf(stderr, "check long: %d\n", check);
            if (check == 1) {
                if (*(config_entry[i].token + 1) == '-' || check == 1)
                    token_format = "\t%-5s\t%-25s\t%-25s\n";
                else
                    token_format = "\t%-5s\t-%-25s\t%-25s\n";
                fprintf(stderr,
                        token_format,
                        config_entry[i+1].token,
                        config_entry[i].token,
                        config_entry[i].description);
                i++;
            } else {
                if (config_entry[i].opt == no_print_in_help_menu || config_entry[i].description == NULL) continue;
                if (*(config_entry[i].token + 1) == '-')
                    token_format = "\t%-5s\t%-25s\t%-25s\n";
                else
                    token_format = "\t%-5s\t-%-25s\t%-25s\n";
                fprintf(stderr, token_format, "", config_entry[i].token, config_entry[i].description);
            }
        }
        return 1;
    } else {
